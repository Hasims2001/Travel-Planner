...
def singlePackage(id):
    try:
        single_dest = db.session.get(Package, id)
        if(request.method == 'GET'):
            return jsonify({'issue': False, 'message': f"Package id {id}.", "Package": single_dest})
...
def singlePlans(id):
    try:
        ...
        if(request.method == 'GET'):
            return jsonify({'issue': False, 'message': f"{id} plan", 'plan': plan})
